...
public class MusicBandSpecifications {
	...
	public static Specification<MusicBand> establishmentDateBefore(Date date) {
		return (root, query, cb) -> date == null ? null : cb.lessThan(root.get("establishmentDate"), date);
	}

	public static Specification<MusicBand> establishmentDateAfter(Date date) {
		return (root, query, cb) -> date == null ? null : cb.greaterThan(root.get("establishmentDate"), date);
	}

	public static Specification<MusicBand> withFilter(MusicBandFilter filter) {
		return Specification.where(hasName(filter.getName())).and(hasDescription(filter.getDescription())).and(hasGenre(filter.getGenre())).and(hasFrontManName(filter.getFrontManName())).and(hasBestAlbumName(filter.getBestAlbumName())).and(participantsBetween(filter.getMinParticipants(), filter.getMaxParticipants())).and(singlesBetween(filter.getMinSingles(), filter.getMaxSingles())).and(albumsCountBetween(filter.getMinAlbumsCount(), filter.getMaxAlbumsCount())).and(coordinateXBetween(filter.getMinCoordinateX(), filter.getMaxCoordinateX())).and(coordinateYBetween(filter.getMinCoordinateY(), filter.getMaxCoordinateY())).and(establishmentDateAfter(filter.getEstablishmentDateAfter())).and(establishmentDateBefore(filter.getEstablishmentDateBefore()));
	}
