...
INSERT INTO music_band (name, coordinates_id, genre, number_of_participants, singles_count, description, best_album_id,
                        albums_count, establishment_date, front_man_id, created_by, created_date, last_modified_by, last_modified_date)
SELECT
    CASE (random() * 7)::int
        WHEN 0 THEN 'The ' ||
    ...
    (random() * 199 + 1)::int,
    (random() * 15 + 1)::bigint,
    timestamp '1970-01-01' + random() * (timestamp '2020-01-01' - timestamp '1970-01-01'),
    (random() * 199 + 1)::int,
    'system',
    CURRENT_TIMESTAMP - (random() * 365)::integer * INTERVAL '1 day',
    'system',
